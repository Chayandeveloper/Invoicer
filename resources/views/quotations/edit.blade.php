...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Edit Quotation</h1>
                <p class="text-sm text-gray-500 font-medium">Update professional quotation #{{ $quotation->quotation_number }}</p>
            </div>
        </div>

        <form action="{{ route('quotations.update', $quotation->id) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white p-6 sm:p-8 rounded-2xl border border-gray-100 shadow-sm space-y-6">
                        ...
                        <select id="business_select" onchange="populateBusiness()"
                            ...
                            @foreach($businesses as $business)
                                <option value="{{ $business->id }}" data-name="{{ $business->name }}"
                                    ...
                                    {{ $quotation->sender_name == $business->name ? 'selected' : '' }}>
                                    {{ $business->name }}
                                </option>
                            @endforeach
                        </select>
                        <input type="hidden" name="sender_logo" id="logo" value="{{ $quotation->sender_logo }}">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="sender_name" id="sender_name" value="{{ $quotation->sender_name }}" required
                            class="w-full border-gray-100 bg-gray-50 rounded-xl text-xs font-bold focus:ring-primary focus:border-primary p-4">
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1 tracking-widest">Website</label>
                        <input type="text" name="sender_website" id="sender_website" value="{{ $quotation->sender_website }}"
                            class="w-full border-gray-100 bg-gray-50 rounded-xl text-xs font-bold focus:ring-primary focus:border-primary p-4">
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1 tracking-widest">Phone Number</label>
                        <input type="text" name="sender_phone" id="sender_phone" value="{{ $quotation->sender_phone }}"
                            class="w-full border-gray-100 bg-gray-50 rounded-xl text-xs font-bold focus:ring-primary focus:border-primary p-4">
                    </div>
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="sender_address" id="sender_address" rows="2"
                            class="w-full border-gray-100 bg-gray-50 rounded-xl text-xs font-bold focus:ring-primary focus:border-primary p-4">{{ $quotation->sender_address }}</textarea>
                    </div>
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="bank_details" id="bank_details" rows="3"
                            class="w-full border-gray-100 bg-gray-50 rounded-xl text-xs font-bold focus:ring-primary focus:border-primary p-4">{{ $quotation->bank_details }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="bg-white p-6 sm:p-8 rounded-2xl border border-gray-100 shadow-sm space-y-6">
                    ...
                    <div>
                        <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1 tracking-widest">Select Saved Client</label>
                        <select id="client_select" onchange="populateClient()"
                            ...
                            @foreach($clients as $client)
                                <option value="{{ $client->id }}" data-name="{{ $client->name }}"
                                    ...
                                    {{ $quotation->client_name == $client->name ? 'selected' : '' }}>
                                    {{ $client->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-4">
                        <input type="hidden" name="client_logo" id="client_logo" value="{{ $quotation->client_logo }}">
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1 tracking-widest">Client Name</label>
                            <input type="text" name="client_name" id="client_name" value="{{ $quotation->client_name }}" required
                                class="w-full border-gray-100 bg-gray-50 rounded-xl text-xs font-bold focus:ring-primary focus:border-primary p-4">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1 tracking-widest">Phone Number</label>
                            <input type="text" name="client_phone" id="client_phone" value="{{ $quotation->client_phone }}"
                                class="w-full border-gray-100 bg-gray-50 rounded-xl text-xs font-bold focus:ring-primary focus:border-primary p-4">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1 tracking-widest">Address</label>
                            <textarea name="client_address" id="client_address" rows="3"
                                class="w-full border-gray-100 bg-gray-50 rounded-xl text-xs font-bold focus:ring-primary focus:border-primary p-4">{{ $quotation->client_address }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 sm:p-8 rounded-2xl border border-gray-100 shadow-sm space-y-6 text-gray-800">
                    <div class="flex items-center gap-3 mb-2">
                        ...
                        <h2 class="text-lg font-black text-gray-800 uppercase tracking-widest text-[11px]">Quotation Details</h2>
                    </div>

                    <div>
                        <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1 tracking-widest">Quotation Number</label>
                        <input type="text" name="quotation_number" value="{{ $quotation->quotation_number }}" required
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1 tracking-widest">Date</label>
                            <input type="date" name="quotation_date" value="{{ $quotation->quotation_date }}" required
                                class="w-full border-gray-100 bg-gray-50 rounded-xl text-xs font-bold focus:ring-primary focus:border-primary p-4">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1 tracking-widest">Expiry Date</label>
                            <input type="date" name="expiry_date" value="{{ $quotation->expiry_date }}"
                                class="w-full border-gray-100 bg-gray-50 rounded-xl text-xs font-bold focus:ring-primary focus:border-primary p-4">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="tax_rate" id="tax_rate" value="{{ $quotation->tax_rate }}" min="0" step="0.01"
                    ...
                    <div id="items-list" class="space-y-4">
                        ...
                        <div class="item-row bg-gray-50/50 p-4 rounded-2xl border border-gray-100 lg:bg-transparent lg:p-0 lg:border-0 lg:grid lg:grid-cols-12 lg:gap-4 lg:items-center">
                            ...
                            <div class="lg:col-span-2 mt-4 lg:mt-0 pt-4 lg:pt-0 border-t border-gray-100 lg:border-0 flex justify-between lg:justify-end items-center text-right">
                                ...
                                <span class="amount font-black text-gray-900 text-sm italic w-full">Rs. {{ number_format($item->amount, 2) }}</span>
                            </div>
            ...
            <div class="flex justify-end">
                <div class="w-full lg:w-1/2 bg-white p-8 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                    <div class="flex justify-between items-center pb-2 border-b border-gray-50">
                        ...
                        <span id="subtotal-display" class="font-bold text-gray-700">Rs. {{ number_format($quotation->subtotal, 2) }}</span>
                    </div>
                    <div class="flex justify-between items-center pb-2 border-b border-gray-50">
                        <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Item Tax</span>
                        <span id="item-tax-display" class="font-bold text-gray-700">Rs. 0.00</span>
                    </div>
                    <div class="flex justify-between items-center pb-2 border-b border-gray-50">
                        <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Global Tax</span>
                        <span id="global-tax-display" class="font-bold text-gray-700">Rs. 0.00</span>
                    </div>
                    <div class="flex justify-between items-center pt-2">
                        <span class="text-xs font-black text-gray-900 uppercase tracking-widest">Estimated Total</span>
                        <span id="total-display" class="text-3xl font-black text-primary italic tracking-tighter">Rs. {{ number_format($quotation->total, 2) }}</span>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function addItem() {
            const row = `
                    <div class="item-row bg-gray-50/50 p-4 rounded-2xl border border-gray-100 lg:bg-transparent lg:p-0 lg:border-0 lg:grid lg:grid-cols-12 lg:gap-4 lg:items-center">
                        ...
                        <div class="lg:col-span-2 mt-4 lg:mt-0 pt-4 lg:pt-0 border-t border-gray-100 lg:border-0 flex justify-between lg:justify-end items-center text-right">
                            <label class="lg:hidden block text-[9px] font-black text-gray-400 uppercase tracking-widest">Subtotal</label>
                            <span class="amount font-black text-gray-900 text-sm italic w-full">Rs. 0.00</span>
                        </div>
                        <div class="lg:col-span-1 flex justify-end">
                            <button type="button" onclick="removeItem(this)" class="p-2 text-red-400 hover:text-red-600 transition bg-white lg:bg-gray-50 rounded-xl">
                                <i class="fas fa-trash-alt"></i>
        ...
        function calculateTotal() {
            ...
            document.querySelectorAll('.item-row').forEach(row => {
                ...
                row.querySelector('.amount').textContent = 'Rs. ' + lineAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 });
            });
            ...
            document.getElementById('subtotal-display').textContent = 'Rs. ' + subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 });
            document.getElementById('item-tax-display').textContent = 'Rs. ' + totalItemTax.toLocaleString('en-IN', { minimumFractionDigits: 2 });
