...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Create Proforma Invoice</h1>
                <p class="text-sm text-gray-500 font-medium">Issue a new professional proforma invoice</p>
            </div>
        </div>

        <form action="{{ route('proforma_invoices.store') }}" method="POST" class="space-y-8" enctype="multipart/form-data">
            @csrf
            ...
            <div class="bg-white p-6 sm:p-8 rounded-2xl border border-gray-100 shadow-sm space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        ...
                        <input type="hidden" name="business_profile" id="business_profile">
                ...
                <div>
                    ...
                    <p class="text-[9px] text-gray-400 mt-2 ml-1 font-bold italic uppercase tracking-tighter">This will be
                        converted to a QR code on the proforma invoice.</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="bg-white p-6 sm:p-8 rounded-2xl border border-gray-100 shadow-sm space-y-6 text-gray-800">
                    <div class="flex items-center gap-3 mb-2">
                        ...
                        <h2 class="text-lg font-black text-gray-800 uppercase tracking-widest text-[11px]">Proforma Details
                        </h2>
                    </div>

                    <div>
                        <label
                            class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1 tracking-widest">Proforma
                            Number</label>
                        <input type="text" name="invoice_number" value="PROF-{{ rand(1000, 9999) }}" required
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label
                                class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1 tracking-widest">Expiry
                                Date</label>
    ...
    <script>
        ...
        function addItem() {
            const row = `
                        <div class="item-row lg:grid lg:grid-cols-12 lg:gap-4 lg:items-center mt-4">
                            <div class="lg:col-span-4"><input type="text" name="items[${itemCount}][description]" required class="w-full border-gray-100 bg-gray-50 rounded-xl text-xs font-bold p-3"></div>
                            <div class="lg:col-span-2"><input type="number" name="items[${itemCount}][quantity]" required min="1" value="1" class="qty w-full border-gray-100 bg-gray-50 rounded-xl text-xs font-bold p-3" oninput="calculateTotal()"></div>
                            <div class="lg:col-span-2"><input type="number" name="items[${itemCount}][unit_price]" required min="0" step="0.01" class="price w-full border-gray-100 bg-gray-50 rounded-xl text-xs font-bold p-3 text-right" oninput="calculateTotal()"></div>
                            <div class="lg:col-span-1"><input type="number" name="items[${itemCount}][tax_rate]" min="0" step="0.01" value="0" class="tax-rate w-full border-gray-100 bg-gray-50 rounded-xl text-xs font-bold p-3 text-right" oninput="calculateTotal()"></div>
                            <div class="lg:col-span-2 text-right"><span class="amount font-black text-gray-900 text-sm italic">₹0.00</span></div>
                            <div class="lg:col-span-1 flex justify-end"><button type="button" onclick="this.closest('.item-row').remove();calculateTotal()" class="p-2 text-red-400 hover:text-red-600"><i class="fas fa-trash-alt"></i></button></div>
                        </div>`;
