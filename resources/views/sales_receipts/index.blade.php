...
@section('content')
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-6">
        <div>
            <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Sales Receipts</h1>
            <p class="text-sm text-gray-500 font-medium">Manage and download your manual sales receipts</p>
        </div>
        <a href="{{ route('sales-receipts.create') }}"
            ...
            <i class="fas fa-plus"></i> Add Sales Receipt
        </a>
    </div>
            ...
            <table class="w-full text-left text-sm text-gray-600 min-w-[800px]">
                <thead
                    ...
                    <tr>
                        ...
                        <th class="px-6 py-5">Item / Description</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($receipts as $receipt)
                        <tr class="hover:bg-gray-50/50 transition">
                            <td class="px-6 py-4">
                                <div class="font-bold text-gray-900 italic tracking-tight">#{{ $receipt->receipt_number }}</div>
                                <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                                    {{ $receipt->client_name ?? 'Valued Client' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="font-bold text-gray-900">
                                    {{ \Carbon\Carbon::parse($receipt->receipt_date)->format('M d, Y') }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <span
                            ...
                            <td class="px-6 py-4">
                                <div class="text-xs font-bold text-gray-600 line-clamp-1 max-w-[200px]">
                                    {{ $receipt->item_description ?? 'General Sales' }}
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="font-black text-gray-900 text-base">Rs. {{ number_format($receipt->amount, 2) }}</div>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('sales-receipts.show', $receipt->id) }}"
                                    ...
                                    <form action="{{ route('sales-receipts.destroy', $receipt->id) }}" method="POST" class="inline"
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-20 text-center text-gray-400">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="font-black text-gray-500 uppercase tracking-widest text-xs">No receipts recorded yet</p>
                                    <p class="text-[10px] mt-2 font-medium">Record your first sales receipt to manage transactions.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
