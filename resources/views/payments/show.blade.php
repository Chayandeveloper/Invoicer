...
<div class="max-w-2xl mx-auto px-4 py-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-8 no-print">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-xl font-black text-gray-900 tracking-tight">Receipt Detail</h1>
                <p class="text-xs text-gray-500 font-bold uppercase tracking-widest">#{{ $payment->receipt_number }}</p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-xl overflow-hidden animate-slide-in">
        ...
        <div class="bg-primary px-8 py-7 relative overflow-hidden">
            ...
            <div class="relative z-10 flex items-center justify-between">
                ...
                <div class="flex items-center gap-4">
                    ...
                    @endif
                    <div>
                        <p class="text-white font-black text-base tracking-tight uppercase">{{ $business->name ?? 'Receipt' }}</p>
                        <p class="text-white/60 text-[10px] uppercase tracking-[0.15em] font-bold mt-0.5">{{ $business->tagline ?? 'Official Payment Receipt' }}</p>
                    </div>
                </div>
                {{-- Receipt Meta --}}
                <div class="text-right">
                    <p class="text-white/40 text-[9px] uppercase tracking-[0.2em] font-bold">Receipt</p>
                    <p class="text-white font-black text-sm mt-0.5">{{ \Carbon\Carbon::parse($payment->payment_date)->format('d M, Y') }}</p>
                    <p class="text-white/50 text-[10px] font-mono mt-0.5">#{{ $payment->receipt_number }}</p>
                </div>
            </div>
        ...
        <div class="px-8 py-7 space-y-6">
            ...
            {{-- Amount --}}
            <div class="rounded-2xl border border-gray-100 bg-gray-50 py-8 px-6 text-center">
                ...
                <div class="flex items-baseline justify-center gap-2">
                    ...
                    <span class="text-6xl font-black text-gray-900 tracking-tighter leading-none">{{ number_format($payment->amount, 2) }}</span>
                </div>
                <div class="mt-4">
                    <span class="inline-flex items-center gap-1.5 bg-green-50 text-primary border border-primary/15 rounded-full px-4 py-1.5 text-[10px] font-black uppercase tracking-[0.12em]">
                        <i class="fas fa-check-circle text-[10px]"></i> Payment Verified
                    </span>
                </div>
            </div>
            ...
            @if($payment->invoice)
            <div>
                <p class="text-[9px] font-black uppercase tracking-[0.15em] text-gray-400 mb-3">Balance Breakdown</p>
                <div class="grid grid-cols-3 divide-x divide-gray-100 rounded-xl border border-gray-100 overflow-hidden">
                    <div class="bg-white py-4 px-5 text-center">
                        <p class="text-[9px] font-black uppercase tracking-[0.12em] text-gray-400 mb-1.5">Original Total</p>
                        <p class="text-base font-black text-gray-900">Rs. {{ number_format($payment->invoice->total, 2) }}</p>
                    </div>
                    <div class="bg-white py-4 px-5 text-center">
                        <p class="text-[9px] font-black uppercase tracking-[0.12em] text-gray-400 mb-1.5">Total Paid</p>
                        <p class="text-base font-black text-primary">Rs. {{ number_format($payment->invoice->paid_amount, 2) }}</p>
                    </div>
                    <div class="bg-white py-4 px-5 text-center">
                        <p class="text-[9px] font-black uppercase tracking-[0.12em] text-gray-400 mb-1.5">Remaining</p>
                        <div class="flex items-center justify-center gap-1.5">
                            <p class="text-base font-black {{ $payment->invoice->remaining_balance > 0 ? 'text-orange-500' : 'text-gray-400' }}">
                                Rs. {{ number_format($payment->invoice->remaining_balance, 2) }}
                            </p>
                            @if($payment->invoice->remaining_balance == 0)
                                <span class="px-1.5 py-0.5 bg-green-100 text-green-700 text-[8px] font-black uppercase rounded-full">Settled</span>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <div class="flex flex-wrap gap-3">
                <div class="flex items-center gap-3 bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 flex-1 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-sm font-black text-gray-900 truncate mt-0.5">{{ $payment->payment_method }}</p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3 bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 flex-1 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-sm font-mono font-bold text-gray-900 truncate mt-0.5">{{ $payment->reference_number }}</p>
                    </div>
                </div>
                @endif
                ...
                <div class="flex items-center gap-3 bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 flex-1 min-w-0">
                    ...
                    <div class="min-w-0">
                        <p class="text-[9px] font-black uppercase tracking-[0.12em] text-gray-400">Invoice Ref</p>
                        <p class="text-sm font-black text-gray-900 truncate mt-0.5">{{ $payment->invoice->invoice_number }}</p>
                    </div>
                </div>
                @endif
            </div>
            ...
            <div class="bg-amber-50/40 border border-amber-100/50 rounded-2xl p-5 relative overflow-hidden">
                ...
                <p class="text-[9px] font-black uppercase tracking-[0.15em] text-amber-600 mb-2">Payer's Notes</p>
                <p class="text-sm text-gray-600 italic font-medium leading-relaxed">"{{ $payment->notes }}"</p>
            </div>
            @endif
            ...
            <div class="flex items-end justify-between pt-5 border-t border-gray-50">
                <div class="space-y-1.5">
                    ...
                    <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest italic leading-relaxed max-w-xs">
                        Official proof of payment. Digitally generated on {{ date('Y-m-d H:i') }}.
                    </p>
                </div>
        ...
        <div class="bg-primary py-3 text-center">
            <p class="text-[9px] text-white/60 font-bold uppercase tracking-[0.25em]">Thank you for your prompt payment</p>
        </div>
    ...
    <div class="mt-10 flex justify-center gap-10 no-print">
        ...
        <a href="{{ route('payments.index') }}" class="group text-[10px] font-black text-gray-400 hover:text-primary uppercase tracking-[0.2em] transition-all flex items-center gap-2">
...
<style>
    ...
    @media print {
        ...
        .bg-primary {
            background-color: #0C8D5D !important;
            -webkit-print-color-adjust: exact;
            print-color-adjust: exact;
        }
        .text-primary { color: #0C8D5D !important; -webkit-print-color-adjust: exact; }
