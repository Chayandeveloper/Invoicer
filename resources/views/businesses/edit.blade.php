...
@section('content')
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white p-6 sm:p-10 rounded-2xl border border-gray-100 shadow-sm">
            <form id="edit-business-form" action="{{ route('businesses.update', $business->id) }}" method="POST" enctype="multipart/form-data"
                class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    <div>
                        <label
                            ...
                            Name</label>
                        <input type="text" name="name" value="{{ old('name', $business->name) }}" required disabled
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            <label
                                ...
                                Address</label>
                            <input type="email" name="email" value="{{ old('email', $business->email) }}" disabled
                        ...
                        <div>
                            <label
                                ...
                                Number</label>
                            <input type="text" name="phone" value="{{ old('phone', $business->phone) }}" disabled
                    ...
                    <div>
                        <label
                            ...
                            URL</label>
                        <input type="text" name="website" value="{{ old('website', $business->website) }}" disabled
                            class="w-full border-gray-100 bg-gray-50 rounded-xl text-xs font-bold focus:ring-primary focus:border-primary p-4 @error('website') border-red-500 @enderror disabled:opacity-60 disabled:cursor-not-allowed"
                    ...
                    <div>
                        ...
                        <textarea name="address" rows="3" disabled
                            ...
                            placeholder="Enter full business address">{{ old('address', $business->address) }}</textarea>
                        @error('address')
                            <p class="text-[10px] text-red-500 mt-1 font-bold italic">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1 tracking-widest">Bank
                            Details</label>
                        <textarea name="bank_details" rows="3" disabled
                            class="w-full border-gray-100 bg-gray-50 rounded-xl text-xs font-bold focus:ring-primary focus:border-primary p-4 @error('bank_details') border-red-500 @enderror disabled:opacity-60 disabled:cursor-not-allowed"
                            placeholder="Acc Name, Acc Number, IFSC/BIC, Branch...">{{ old('bank_details', $business->bank_details) }}</textarea>
                    </div>
